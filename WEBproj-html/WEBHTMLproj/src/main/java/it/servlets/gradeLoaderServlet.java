...
public class gradeLoaderServlet extends ThymeleafServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (Connection conn = DataBaseConnection.getConnection()){
			...
			String sql = "SELECT students.name AS sname, students.surname, students.email, results.*, exams.*, c.* FROM (students NATURAL JOIN results NATURAL JOIN exams) INNER JOIN courses c ON c.idCourse = exams.idCourse WHERE c.idCourse = ? AND idExam = ?  AND idStudent = ? AND (status = 'published' OR status = 'rejected');";
